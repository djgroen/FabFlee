...
def load_conflict(conflict_name):     # Syntax: fab localhost load_conflict:conflict_name
  ...
  local(template("mkdir -p %s/conflict_data/active_conflict" % (env.localroot)))
  local(template("cp %s/conflict_data/%s/*.csv %s/conflict_data/active_conflict/") % (env.localroot, conflict_name, env.localroot))
  local(template("mkdir -p %s/conflict_data/active_conflict/source_data" % (env.localroot)))
  local(template("cp %s/conflict_data/%s/source_data/*.csv %s/conflict_data/active_conflict/source_data/") % (env.localroot, conflict_name, env.localroot))
...
def change_capacities(**capacities):     # Syntax: fab localhost change_capacities:camp_name=capacity(,camp_name2=capacity2)
  ...
  import csv
  r = csv.reader(open("%s/conflict_data/active_conflict/locations.csv" % (env.localroot)))
  lines = [l for l in r]
  ...
  writer = csv.writer(open("%s/conflict_data/active_conflict/locations.csv" % (env.localroot), "w"))
...
def add_camp(camp_name, region=" ", country=" ", lat=0.0, lon=0.0):    # Syntax: fab localhost add_camp:camp_name,region,country(,lat,lon)
  ...
  with open("%s/conflict_data/active_conflict/locations.csv" % (env.localroot), "a") as new_csv:
...
def close_camp(camp_name, country, closure_start=0, closure_end=-1):     # Syntax: fab localhost close_camp:camp_name,country(,closure_start,closure_end)
  ...
  import csv
  r = csv.reader(open("%s/conflict_data/active_conflict/closures.csv" % (env.localroot))) # Here your csv file
  lines = [l for l in r]
  camp_found = False
  ...
  writer = csv.writer(open("%s/conflict_data/active_conflict/closures.csv" % (env.localroot), "w"))
  writer.writerows(lines)
...
def close_border(country1, country2, closure_start=0, closure_end=-1):     # Syntax: fab localhost close_border:country1,country2(,closure_start,closure_end)
  ...
  r = csv.reader(open("%s/conflict_data/active_conflict/closures.csv" % (env.localroot)))
  lines = [l for l in r]
  border_found = False
...
def redirect(source, destination):    # Syntax: fab localhost redirect:location_name1,location_name2
  ...
  r = csv.reader(open("%s/conflict_data/active_conflict/routes.csv" % (env.localroot)))
  lines = [l for l in r]
  ...
  writer = csv.writer(open("%s/conflict_data/active_conflict/routes.csv" % (env.localroot), "w"))
  writer.writerows(lines)
...
def clear_active_conflict():     # Syntax: fab localhost clear_active_conflict
  ...
  local(template("rm -rf %s/conflict_data/active_conflict/" % (env.localroot)))
...
def instantiate(conflict_name):      # Syntax: fab localhost instantiate:conflict_name
  ...
  local(template("mkdir -p %s/config_files/%s" % (env.localroot, conflict_name)))
  local(template("mkdir -p %s/config_files/%s/input_csv" % (env.localroot, conflict_name)))
  local(template("cp %s/conflict_data/active_conflict/*.csv %s/config_files/%s/input_csv") % (env.localroot, env.localroot, conflict_name))
  local(template("cp %s/conflict_data/active_conflict/commands.log.txt %s/config_files/%s/") % (env.localroot, env.localroot, conflict_name))
  local(template("mkdir -p %s/config_files/%s/source_data" % (env.localroot, conflict_name)))
  local(template("cp %s/conflict_data/active_conflict/source_data/*.csv %s/config_files/%s/source_data") % (env.localroot, env.localroot, conflict_name))
  local(template("cp %s/config_files/flee/run.py %s/config_files/%s/run.py") % (env.localroot, env.localroot, conflict_name))

# Test Functions
from test_fabflee import *
...
def test_sensitivity(config,**args):     #Syntax: fab localhost test_sensitivity:flee_conflict_name,simulation_period=number,name=MaxMoveSpeed,values=50-100-200
  ...
  for v in values:
    ...
    csvfile = open('%s/config_files/%s_%s_%s/simsetting.csv' % (env.localroot, config, args["name"], v), "wb")
