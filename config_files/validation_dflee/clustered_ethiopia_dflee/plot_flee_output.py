...
def plot_camps(data: pd.DataFrame, config: str, output: str) -> None:
    ...
    for i in range(len(cols)):
        ...
        (label2,) = plt.plot(data_filtered.index, y2, "b", linewidth=5, label="{} IOM data".format(name[0]))
...
def plot_numagents(data: pd.DataFrame, config: str, output: str) -> None:
    ...
    if "refugee_debt" in data.columns:
        ...
        plt.legend(
            labels=["Total Refugees", "Refugees in Shelters", "Raw IOM Count", "Refugee Debt"],
        )

    else:
        data.loc[
            :,
            ["total refugees (simulation)", "refugees in camps (IOM)", "raw IOM refugee count"],
        ].plot(linewidth=5)
    ...
    plt.savefig("{}/numagents.pdf".format(output), bbox_inches = 'tight')
