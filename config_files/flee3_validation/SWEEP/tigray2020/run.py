...
#Tigray (simulation),Tigray (UNHCR),Tigray_diff"
  ...
  for t in range(0,end_time):
    if t == 0:
      ...
      ig.AddNewConflictZones(e, 1)
      ig.AddNewConflictZones(e, 2)
    elif t > 0:
      ig.AddNewConflictZones(e, t + 2)
...
#, (e.numAgents()-refugees_in_camps_sim), loc_data[i],(float(int(sum(loc_data)-refugees_in_camps_sim))/float(loc_data[i])))
    else:
      output += ",0,0,0,0,0,0"
