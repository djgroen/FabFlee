...
def run_ssudan_paper():
  for postfix in ["default","reg","links","ccamp","cborder","redirect","adjumani1","adjumani2"]:  
    ...
    test_sensitivity("ssudan_%s" % (postfix),simulation_period=604,name="AwarenessLevel",values="0-1-2")
    for i in [0, 1, 2]:
      plot_output("ssudan_%s_AwarenessLevel_%s_localhost_16" % (postfix, i),"out")
